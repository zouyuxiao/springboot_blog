...
public class CommentController extends BaseApiController {
    ...
    // 添加评论
    @PostMapping("add")
    public Map<String,Object> add(@RequestParam String content, @RequestParam Long uId,@RequestParam Long contentId){
...
//        if (uId == null) return onBadResp( "用户id不能为空" );
        ...
        if (commentService.add( comment ) > 0) return onSuccessRep( "评论成功" );
        return onBadResp( "添加失败" );
    }

    @GetMapping("selectReply/{id}")
    @ResponseBody
    public Map<String,Object> selectReply(@PathVariable Long id,
                                          @RequestParam(defaultValue = "1")Integer page_num,
                                          @RequestParam(defaultValue = "10")Integer page_size){
        PageHelper.startPage( page_num,page_size );
        return onDataResp( new MyPageInfo<Reply>( commentService.selectReply( id )));
    }
}
